...
class Gridbox_Pro_Header_Search {
	...
	static function enqueue_script() {
		...
		if ( ( true === $theme_options['header_search'] || is_customize_preview() ) && ! self::is_amp() ) :
			...
			wp_enqueue_script( 'gridbox-pro-header-search', GRIDBOX_PRO_PLUGIN_URL . 'assets/js/header-search.min.js', array(), '20220121', true );
	...
	static function add_header_search( $items, $args ) {
		...
		if ( true === $theme_options['header_search'] || is_customize_preview() ) :
			...
			$items .= '<button class="header-search-icon" aria-label="' . esc_attr__( 'Open search form', 'gridbox-pro' ) . '" aria-expanded="false" aria-controls="header-search-dropdown" ' . self::amp_search_toggle() . '>';
