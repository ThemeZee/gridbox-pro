...
class Gridbox_Pro_Magazine_Posts_Boxed_Widget extends WP_Widget {
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $settings['category'] > 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'gridbox-pro' ), get_cat_name( $settings['category'] ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e( 'Post Layout:', 'gridbox-pro' ); ?></label><br/>
			<select id="<?php echo $this->get_field_id( 'layout' ); ?>" name="<?php echo $this->get_field_name( 'layout' ); ?>">
				<option <?php selected( $settings['layout'], 'horizontal' ); ?> value="horizontal" ><?php esc_html_e( 'Horizontal Arrangement', 'gridbox-pro' ); ?></option>
